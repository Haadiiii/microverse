...
describe('index.html', () => {
  ...
  describe("head", () => {
    ...
    it("has link to JS script", () => {
      let head = document.querySelector('head')
      expect(head).to.contain('script')
    });
  });
  ...
  describe("JavaScript link", () => {
    it("has attribute src equals to challenge.js'", () => {
      let link = document.querySelector('head script')
      expect(link).to.have.attribute('src', 'challenge.js')
    });
  });
...
describe('challenge.js', function () {

  describe('findAllUnorderedListElements', function () {
    it('returns elements of unordered list with social media names as text', function () {
      let elements = challenge.findAllUnorderedListElements();
      expect(elements[0]).to.have.text('Facebook');
      expect(elements[1]).to.have.text('Instagram');
      expect(elements[2]).to.have.text('Twitter');
    });
  });

  describe('prepareProjects', function () {
    it('returns all projects passed as arguments', function () {
      let projectsOutput = challenge.prepareProjects('Amazing project', 'Awesome project', 'Beautiful project');
      console.log(projectsOutput)
      expect(projectsOutput).to.include('Amazing project');
      expect(projectsOutput).to.include('Awesome project');
      expect(projectsOutput).to.include('Beautiful project');
    });

    it('encloses projects longer than 20 chars inside a <p> tag', function () {
      let projectsOutput = challenge.prepareProjects('Amazing project', 'Awesome project', 'Beautiful project with more than twenty chars');
      expect(projectsOutput).to.include('<p>Beautiful project with more than twenty chars</p>');
    });

    it('encloses projects NOT longer than 20 chars inside a <span> tag', function () {
      let projectsOutput = challenge.prepareProjects('Amazing project', 'Awesome project', 'Beautiful project with more than twenty chars');
      expect(projectsOutput).to.include('<span>Awesome project</span>');
      expect(projectsOutput).to.include('<span>Awesome project</span>');
    });

    it('adds a <br> tag after each project', function () {
      let projectsOutput = challenge.prepareProjects('Amazing project', 'Awesome project', 'Beautiful project');
      tagOcurrences = (projectsOutput.match(/<br>/g) || []).length
      expect(tagOcurrences).to.equal(3);
    });
  });
});
